...
export default function ActionItemTracking() {
  const items = [
    { ref: "MR-2026-01", action: "Upgrade backup power system (UPS)", due: "2026-07-01", owner: "Maintenance Team", status: "In progress" },
    { ref: "MR-2026-02", action: "Deploy Barcode scanners for pre-exam check", due: "2026-06-15", owner: "IT Manager", status: "Completed" }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Management Review Action items (ISO 15189 §8.9.3)</h3>
      <p style={S.subtitle}>Mitigation controls tracking log</p>

      <table style={{ ...S.table, marginTop: 14 }}>
        <thead>
          <tr>
            <th style={S.th}>Meeting Ref</th>
            <th style={S.th}>Action Item</th>
        ...
        <tbody>
          {items.map((it, i) => (
            <tr key={i}>
              <td style={S.td}><code>{it.ref}</code></td>
              <td style={S.td}><strong>{it.action}</strong></td>
              <td style={S.td}>{it.due}</td>
              <td style={S.td}>{it.owner}</td>
              <td style={S.td}>
                <span style={S.badge(it.status === "Completed" ? "#E6F4EA" : "#EFF6FF", it.status === "Completed" ? "#137333" : "#1E40AF")}>
                  {it.status}
                </span>
              </td>
            </tr>
