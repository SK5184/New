...
export default function DepartmentWiseAuditAnalysis() {
  const list = [
    { dept: "Microbiology", compliance: 95.0, status: "Compliant" },
    { dept: "Biochemistry", compliance: 98.4, status: "Compliant" },
    { dept: "Haematology", compliance: 100.0, status: "Excellent" },
    { dept: "Serology", compliance: 92.5, status: "Compliant" }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Department Audit compliance matrix</h3>
      <p style={S.subtitle}>Compliance scores resolved during internal audits</p>
      
      <table style={{ ...S.table, marginTop: 14 }}>
        <thead>
          <tr>
            <th style={S.th}>Department</th>
            <th style={S.th}>Compliance Score</th>
        ...
        <tbody>
          {list.map((l, i) => (
            <tr key={i}>
              <td style={S.td}><strong>{l.dept}</strong></td>
              <td style={S.td}>{l.compliance}%</td>
              <td style={S.td}>
                <span style={S.badge(l.compliance >= 98 ? "#E6F4EA" : "#EFF6FF", l.compliance >= 98 ? "#137333" : "#1E40AF")}>
                  {l.status}
                </span>
              </td>
            </tr>
