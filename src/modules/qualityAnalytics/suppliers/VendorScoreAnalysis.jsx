...
export default function VendorScoreAnalysis() {
  const vendors = [
    { name: "Global Reagents Co", score: 98, status: "Excellent" },
    { name: "Biomed Services Ltd", score: 92, status: "Good" },
    { name: "Apex Diagnostics", score: 78, status: "Audit required" }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Supplier quality evaluations</h3>
      <p style={S.subtitle}>Accuracy of reagents, documentation, and compliance score</p>
      
      <table style={{ ...S.table, marginTop: 14 }}>
        <thead>
          <tr>
            <th style={S.th}>Supplier Name</th>
            <th style={S.th}>Quality Score</th>
        ...
        <tbody>
          {vendors.map((v, i) => (
            <tr key={i}>
              ...
              <td style={S.td}>{v.score}%</td>
              <td style={S.td}>
                <span style={S.badge(v.score >= 90 ? "#E6F4EA" : "#FFF3CD", v.score >= 90 ? "#137333" : "#854F0B")}>
                  {v.status}
                </span>
              </td>
            </tr>
