...
export default function SampleRejectionAnalytics() {
  const reasons = [
    { reason: "Clotted Sample", count: 14, pct: 41 },
    { reason: "Hemolyzed Sample", count: 11, pct: 32 },
    { reason: "Insufficient Volume", count: 6, pct: 18 },
    { reason: "Incorrect Container", count: 3, pct: 9 }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Pre-Examination Sample Rejections</h3>
      <p style={S.subtitle}>Pareto category analysis of rejected specimens</p>

      <div style={{ marginTop: 20 }}>
        {reasons.map((r, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
            <div style={{ width: 140, fontSize: 12, color: "#1E293B" }}>{r.reason}</div>
            <div style={{ flex: 1, height: 12, background: "#F1F5F9", borderRadius: 6, overflow: "hidden", marginRight: 12 }}>
              <div style={{ width: `${r.pct}%`, height: "100%", background: "#0D9488" }} />
            </div>
            <div style={{ width: 60, textAlign: "right", fontSize: 12, fontWeight: "bold" }}>{r.count} ({r.pct}%)</div>
          </div>
        ))}
      </div>
    </div>
