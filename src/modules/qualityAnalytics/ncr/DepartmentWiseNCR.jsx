...
export default function DepartmentWiseNCR() {
  const depts = [
    { label: "Micro", count: 9 },
    { label: "Bioc", count: 12 },
    { label: "Haem", count: 5 },
    { label: "Phleb", count: 14 }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Department-wise NCR Distribution</h3>
      <p style={S.subtitle}>Monthly comparisons of registered deviations</p>

      <div style={{ marginTop: 20 }}>
        <svg viewBox="0 0 400 120" style={{ width: "100%", height: 100 }}>
          {depts.map((d, i) => {
            const x = 50 + i * 90;
            const h = (d.count / 15) * 80;
            const y = 90 - h;
            return (
              <g key={i}>
                <rect x={x - 14} y={y} width="28" height={h} fill="#3B82F6" rx={4} />
                <text x={x} y={y - 6} textAnchor="middle" fontSize="9.5" fontWeight="bold" fill="#3B82F6">{d.count}</text>
                <text x={x} y="110" textAnchor="middle" fontSize="9" fill="#94A3B8">{d.label}</text>
              </g>
            );
          })}
        </svg>
