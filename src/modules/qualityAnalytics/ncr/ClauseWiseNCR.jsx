...
export default function ClauseWiseNCR() {
  const clauses = [
    { label: "Clause 7.2 (Pre-exam)", count: 15 },
    { label: "Clause 7.3 (Exam/QC)", count: 9 },
    { label: "Clause 6.4 (Equipment)", count: 6 },
    { label: "Clause 6.2 (Personnel)", count: 2 }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>ISO 15189:2022 Clause-wise Non-conformances</h3>
      <p style={S.subtitle}>Consolidated clause mapping analysis</p>
      
      <div style={{ marginTop: 20 }}>
        {clauses.map((c, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #F1F5F9", padding: "8px 0" }}>
            <span style={{ fontSize: 12 }}>{c.label}</span>
            <span style={{ fontSize: 12, fontWeight: "bold", color: "#EF4444" }}>{c.count} NCRs</span>
          </div>
        ))}
      </div>
    </div>
