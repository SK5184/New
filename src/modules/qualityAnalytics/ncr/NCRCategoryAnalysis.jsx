...
export default function NCRCategoryAnalysis() {
  const rootCauses = [
    { label: "Process Inefficiency", count: 18, pct: 45, color: "#0D9488" },
    { label: "Human Error", count: 12, pct: 30, color: "#3B82F6" },
    { label: "Equipment Drift", count: 6, pct: 15, color: "#F59E0B" },
    { label: "Environment Factors", count: 4, pct: 10, color: "#EF4444" }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Root Cause category Trends</h3>
      <p style={S.subtitle}>Consolidated categorization of NCR findings</p>

      <div style={{ marginTop: 20 }}>
        {rootCauses.map((r, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
            <div style={{ width: 140, fontSize: 12, color: "#1E293B" }}>{r.label}</div>
            <div style={{ flex: 1, height: 10, background: "#F1F5F9", borderRadius: 5, overflow: "hidden", marginRight: 12 }}>
              <div style={{ width: `${r.pct}%`, height: "100%", background: r.color }} />
            </div>
            <div style={{ width: 50, textAlign: "right", fontSize: 12, fontWeight: "bold" }}>{r.count}</div>
          </div>
        ))}
      </div>
    </div>
