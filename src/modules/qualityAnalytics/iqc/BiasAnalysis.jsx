...
export default function BiasAnalysis({ analyte }) {
  return (
    <div style={S.card}>
      <h3 style={S.title}>Monthly Bias & Accuracy deviation</h3>
      <p style={S.subtitle}>Deviation from peer target mean for {analyte}</p>
      
      <div style={{ marginTop: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #F1F5F9", padding: "8px 0" }}>
          <span>Current Month Bias</span>
          <span style={{ fontWeight: "bold", color: "#0D9488" }}>+1.20%</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #F1F5F9", padding: "8px 0" }}>
          <span>Target Acceptable Bias</span>
          <span>&lt; 2.50%</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0" }}>
          <span>Bias Status</span>
          <span style={{ color: "#0D9488", fontWeight: 600 }}>Acceptable</span>
        </div>
      </div>
    </div>
  );
