...
export default function SigmaMetrics({ analyte }) {
  const tea = 10.0;
  const bias = 1.2;
  const cv = 1.8;
  const sigma = ((tea - bias) / cv).toFixed(2);

  const scoreColor = sigma >= 6 ? "#0D9488" : sigma >= 4 ? "#3B82F6" : "#EF4444";

  return (
    <div style={S.card}>
      <h3 style={S.title}>Analytical Sigma Metrics Scorecard</h3>
      <p style={S.subtitle}>Consolidated precision quality score for {analyte}</p>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 24 }}>
        <div>
          <div style={{ fontSize: 36, fontWeight: 800, color: scoreColor }}>{sigma} σ</div>
          <div style={{ fontSize: 12, fontWeight: 600, color: "#64748B", marginTop: 4 }}>
            {sigma >= 6 ? "World Class Quality" : sigma >= 4 ? "Excellent Quality" : "Action Required"}
          </div>
        </div>
        
        <div style={{ fontSize: 12, color: "#475569", display: "flex", flexDirection: "column", gap: 4 }}>
          <div><strong>TEa Limit:</strong> {tea}%</div>
          <div><strong>Observed Bias:</strong> {bias}%</div>
          <div><strong>Observed CV%:</strong> {cv}%</div>
        </div>
      </div>
    </div>
  );
