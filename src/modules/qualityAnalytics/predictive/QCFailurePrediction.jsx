...
export default function QCFailurePrediction() {
  return (
    <div style={S.card}>
      <h3 style={S.title}>IQC Run Failure alerts</h3>
      <p style={S.subtitle}>Predicts potential failures based on control drift trajectories</p>
      
      <div style={{ padding: "12px 16px", background: "#EFF6FF", borderRadius: 8, border: "0.5px solid #93C5FD", display: "flex", gap: 10, alignItems: "center", marginTop: 14 }}>
        <span style={{ fontSize: 18 }}>ℹ</span>
        <span style={{ fontSize: 12, color: "#1E40AF" }}>All runs are stable. Control deviations are inside standard SD bounds.</span>
      </div>
    </div>
  );
}