...
export default function CAPAEffectiveness() {
  const steps = [
    { title: "CAPA-012: Patient ID", pre: 16, post: 4 },
    { title: "CAPA-014: Pipette Drift", pre: 12, post: 3 },
    { title: "CAPA-015: Courier Delay", pre: 15, post: 5 }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Risk Mitigation effectiveness</h3>
      <p style={S.subtitle}>Initial risk severity vs Residual risk severity</p>

      <div style={{ marginTop: 20 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 11.5, fontWeight: "bold", marginBottom: 6 }}>{s.title}</div>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <span style={{ fontSize: 10, color: "#64748B", width: 30 }}>Initial</span>
              <div style={{ flex: 1, height: 8, background: "#FEE2E2", borderRadius: 4 }}>
                <div style={{ width: `${(s.pre / 25) * 100}%`, height: "100%", background: "#EF4444", borderRadius: 4 }} />
              </div>
              <span style={{ fontSize: 10.5, fontWeight: "bold", color: "#EF4444", width: 20 }}>{s.pre}</span>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 4 }}>
              <span style={{ fontSize: 10, color: "#64748B", width: 30 }}>Residual</span>
              <div style={{ flex: 1, height: 8, background: "#E6FDF5", borderRadius: 4 }}>
                <div style={{ width: `${(s.post / 25) * 100}%`, height: "100%", background: "#0D9488", borderRadius: 4 }} />
              </div>
              <span style={{ fontSize: 10.5, fontWeight: "bold", color: "#0D9488", width: 20 }}>{s.post}</span>
            </div>
          </div>
        ))}
      </div>
