...
export default function OverdueCAPA() {
  const overdues = [
    { id: "CAPA-2026-08", title: "Verify room temperature logger in Serology", date: "2026-06-10", owner: "HOD Serology", delay: "9 days overdue" }
  ];

  return (
    <div style={S.card}>
      <h3 style={S.title}>Overdue CAPA Actions</h3>
      <p style={S.subtitle}>Action items currently exceeding target deadline</p>

      {overdues.length === 0 ? (
        <div style={{ padding: "12px 16px", background: "#E6FDF5", borderRadius: 8, color: "#065F46", fontSize: 12, marginTop: 14 }}>
          ✓ Zero overdue CAPAs. All actions are on schedule.
        </div>
      ) : (
        <table style={{ ...S.table, marginTop: 14 }}>
          <thead>
            <tr>
              <th style={S.th}>CAPA ID</th>
          ...
          <tbody>
            {overdues.map((o, i) => (
              <tr key={i}>
                <td style={S.td}><code>{o.id}</code></td>
                <td style={S.td}><strong>{o.title}</strong></td>
                <td style={S.td}>{o.date}</td>
                <td style={S.td}>{o.owner}</td>
                <td style={S.td}><span style={{ color: "#EF4444", fontWeight: 600 }}>{o.delay}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
