...
// ISO 15189:2022 & ISO 27001:2022 Compliant Kitchen Dashboard
...
export default function KitchenDashboard({ role, userName }) {
    ...
    <div style={S.wrap}>
      <div style={S.sidebar}>
        <div style={{ padding: "14px 16px", borderBottom: "0.5px solid #E0DDD6" }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#065F46" }}>Kitchen</div>
        ...
        <div style={S.navItem(activeTab === "kit_temp_monitoring")} onClick={() => setActiveTab("kit_temp_monitoring")}>
          <span>🌡️</span> <span>Temperature & Humidity Monitoring</span>
        </div>
      </div>
      <div style={S.content}>
        {activeTab === "roster" && (
          <WeeklyDutyRoster department="Kitchen" role={role} userName={userName} />
        )}
        {activeTab === "kit_temp_monitoring" && (
          <TemperatureDashboard department="Kitchen" />
        )}
      </div>
    </div>
  );
