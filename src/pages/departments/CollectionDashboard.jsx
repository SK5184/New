...
// ISO 15189:2022 & ISO 27001:2022 Compliant Collection Dashboard
...
export default function CollectionDashboard({ role, userName }) {
    ...
    <div style={S.wrap}>
      <div style={S.sidebar}>
        <div style={{ padding: "14px 16px", borderBottom: "0.5px solid #E0DDD6" }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#065F46" }}>Collection</div>
        ...
        <div style={S.navItem(activeTab === "coll_temp_monitoring")} onClick={() => setActiveTab("coll_temp_monitoring")}>
          <span>🌡️</span> <span>Temperature & Humidity Monitoring</span>
        </div>
      </div>
      <div style={S.content}>
        {activeTab === "roster" && (
          <WeeklyDutyRoster department="Collection" role={role} userName={userName} />
        )}
        {activeTab === "coll_temp_monitoring" && (
          <TemperatureDashboard department="Collection" />
        )}
      </div>
    </div>
  );
