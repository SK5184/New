...
const S = {
  ...
  navItem: (active) => ({
    ...
    color: active ? "#9D174D" : "#5F5E5A",
    background: active ? "#FDF2F8" : "transparent",
    borderLeft: active ? "4px solid #EC4899" : "4px solid transparent",
  ...
  btn: (bg, color) => ({
    padding: "6px 12px", background: bg || "#EC4899", color: color || "#FDF2F8",
...
const TABS = [
  ...
  { key: "advisory", label: "Advisory Services", icon: "💬", cat: "Examination Protocols" },
  { key: "comparability", label: "Comparability of test methods", icon: "📊", cat: "Examination Protocols" },
  
  { key: "iqc_alinity1", label: "IQC - Alinity CI 1 (AM/PM)", icon: "📈", cat: "Internal Quality Control" },
  { key: "iqc_alinity2", label: "IQC - Alinity CI 2 (AM/PM)", icon: "📈", cat: "Internal Quality Control" },
  { key: "iqc_atellica", label: "IQC - Atellica NEPH 630", icon: "📈", cat: "Internal Quality Control" },
  { key: "iqc_elisa", label: "IQC - Manual ELISA Result", icon: "📈", cat: "Internal Quality Control" },
  { key: "iqc_maglumi", label: "IQC - Maglumi 800", icon: "📈", cat: "Internal Quality Control" },
  { key: "iqc_phadia", label: "IQC - Phadia 250", icon: "📈", cat: "Internal Quality Control" },
  { key: "iqc_vidas", label: "IQC - Vidas Result", icon: "📈", cat: "Internal Quality Control" },
  { key: "lot_verification", label: "Lot to Lot Verification", icon: "🧪", cat: "Internal Quality Control" },
  
  { key: "eqa_viral", label: "EQAS - Viral Markers", icon: "🌐", cat: "External Quality (EQAS)" },
  { key: "eqa_torch", label: "EQAS - Torch/EBV/Mumps", icon: "🌐", cat: "External Quality (EQAS)" },
  { key: "eqa_phospholipids", label: "EQAS - Anti phospholipids", icon: "🌐", cat: "External Quality (EQAS)" },
  { key: "eqa_coagulation", label: "EQAS - Coagulation", icon: "🌐", cat: "External Quality (EQAS)" },
  { key: "eqa_immunology", label: "EQAS - Diagnostic Immunology", icon: "🌐", cat: "External Quality (EQAS)" },
  { key: "eqa_split_sample", label: "Split Sample testing", icon: "➗", cat: "External Quality (EQAS)" },
  { key: "eqa_inter_lab", label: "Inter Lab Comparison", icon: "🔄", cat: "External Quality (EQAS)" },
  
  { key: "maint_autolumo", label: "Autolumo Maintenance", icon: "⚙️", cat: "Equipment Maintenance" },
  ...
  { key: "cal_atellica", label: "Calibration - Atellica", icon: "⚖️", cat: "Calibrations" },
  { key: "cal_maglumi", label: "Calibration - Maglumi", icon: "⚖️", cat: "Calibrations" },
  { key: "cal_vidas", label: "Calibration - MiniVidas", icon: "⚖️", cat: "Calibrations" },
...
export default function SerologyDashboard({ role, userName }) {
  ...
  const [testForm, setTestForm] = useState({
    testName: "", equipment: "Alinity CI 1", method: "CMIA",
    unit: "S/CO", refRanges: "Non-reactive (<1.0)", criticalLow: "Reactive (>1.0)", criticalHigh: "Reactive (>1.0)",
    qcMean: "1.0", qcSD: "0.05"
  });

  const [iqcForm, setIqcForm] = useState({
    testId: "", level: "Level 1", value: "", mean: "", sd: "", lotNumber: "LOT-SR-26"
  });
  ...
  const [lotInput, setLotInput] = useState({
    currentLotName: "LOT-SR-A", newLotName: "LOT-SR-B",
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      const testSnap = await getDocs(query(collection(db, "testMaster"), where("department", "==", "Serology")));
  ...
  const loadTabRecords = useCallback(async () => {
    try {
        ...
        const snap = await getDocs(query(collection(db, "interactiveLogs"), where("department", "==", "Serology"), where("featureKey", "==", `ser_${activeTab}`), orderBy("createdAt", "desc")));
  ...
  const triggerBreakdownReport = async (eqName) => {
    ...
    try {
      await addDoc(collection(db, "actionRequests"), {
        ...
        status: "Open",
        equipment: eqName,
        details: `Reported from Serology: ${details}`,
        createdAt: serverTimestamp(),
        createdBy: userName || "Staff"
      });
  ...
  const visibleItems = TABS.filter(item => featureFlags[`ser_${item.key}`] !== false);
  const categories = ["General & Personnel", "Examination Protocols", "Internal Quality Control", "External Quality (EQAS)", "Equipment Maintenance", "Calibrations", "Pre-Examination & Process"];
    ...
    <div style={S.wrap}>
      ...
      <div style={S.sidebar}>
        <div style={{ padding: "14px 16px", borderBottom: "0.5px solid #E0DDD6" }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#9D174D" }}>Serology Console</div>
          <div style={{ fontSize: 9.5, color: "#EC4899", marginTop: 2, fontWeight: 500 }}>ISO 15189:2022 Monitoring</div>
        </div>
        {categories.map(cat => {
      ...
      <div style={S.content}>
        {/* Weekly Duty Roster */}
        {activeTab === "roster" && (
          <WeeklyDutyRoster department="Serology" role={role} userName={userName} />
        )}
        ...
        {activeTab === "test_master" && (
          <div>
                ...
                <form onSubmit={handleAddTest}>
                  <div style={S.grid(3)}>
                    <div>
                      ...
                      <input style={S.inp} value={testForm.testName} onChange={e => setTestForm(p => ({ ...p, testName: e.target.value }))} placeholder="e.g. HIV 1/2 Ab" required />
                    </div>
                    <div>
                      <span style={S.label}>Equipment/Analyzer</span>
                      <input style={S.inp} value={testForm.equipment} onChange={e => setTestForm(p => ({ ...p, equipment: e.target.value }))} placeholder="e.g. Alinity CI 1" />
                    </div>
                    <div>
                      <span style={S.label}>Methodology</span>
                      <input style={S.inp} value={testForm.method} onChange={e => setTestForm(p => ({ ...p, method: e.target.value }))} placeholder="e.g. CMIA" />
                    </div>
                  </div>
                  <div style={S.grid(4)}>
                    <div>
                    ...
                    <div>
                      ...
                      <input style={S.inp} value={testForm.qcSD} onChange={e => setTestForm(p => ({ ...p, qcSD: e.target.value }))} placeholder="0.05" />
                    </div>
                    <div>
                      <span style={S.label}>Unit</span>
                      <input style={S.inp} value={testForm.unit} onChange={e => setTestForm(p => ({ ...p, unit: e.target.value }))} placeholder="S/CO" />
                    </div>
            ...
            <div style={S.card}>
              <div style={S.cardHeader}><span style={S.cardTitle}>Registered Serology Tests</span></div>
              <div style={S.cardBody}>
                <table style={S.table}>
                  <thead>
                    <tr>
        ...
        {activeTab === "lot_verification" && (
          <div style={S.card}>
            ...
            <div style={S.cardBody}>
              ...
              {lotResult && (
                <div style={{ background: "#FDF2F8", border: "0.5px solid #FBCFE8", borderRadius: 8, padding: 12, fontSize: 12, color: "#9D174D" }}>
                  ...
                  <br />
                  <strong>Mean Bias:</strong> {lotResult.bland?.meanBias?.toFixed(2)}% &nbsp; (NABL Limit: &le; 10% bias for reagent approval)
                </div>
              )}
            </div>
          </div>
            ...
            <div style={S.card}>
              <div style={S.cardHeader}>
                ...
                {activeTab.startsWith("maint_") && (
                  <button style={S.btn("#FFF5F5", "#A32D2D")} onClick={() => triggerBreakdownReport(TABS.find(t => t.key === activeTab)?.label)}>
                ...
                <form onSubmit={handleGenericLogSubmit}>
                  <div style={S.grid(3)}>
                    ...
                    <div>
                      ...
                      <input style={S.inp} value={genericForm.val} onChange={e => setGenericForm(p => ({ ...p, val: e.target.value }))} placeholder="e.g. 4°C, 0.45 OD" />
                    </div>
                  </div>
