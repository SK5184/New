...
// ISO 15189:2022 & ISO 27001:2022 Compliant Purchase Dashboard
...
export default function PurchaseDashboard({ role, userName }) {
    ...
    <div style={S.wrap}>
      <div style={S.sidebar}>
        <div style={{ padding: "14px 16px", borderBottom: "0.5px solid #E0DDD6" }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#065F46" }}>Purchase</div>
        ...
        <div style={S.navItem(activeTab === "pur_temp_monitoring")} onClick={() => setActiveTab("pur_temp_monitoring")}>
          <span>🌡️</span> <span>Temperature & Humidity Monitoring</span>
        </div>
      </div>
      <div style={S.content}>
        {activeTab === "roster" && (
          <WeeklyDutyRoster department="Purchase" role={role} userName={userName} />
        )}
        {activeTab === "pur_temp_monitoring" && (
          <TemperatureDashboard department="Purchase" />
        )}
      </div>
    </div>
  );
